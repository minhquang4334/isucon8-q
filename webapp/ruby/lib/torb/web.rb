...
module Torb
  class Web < Sinatra::Base
    ...
    helpers do
      ...
      def get_event(event_id, login_user_id = nil)
        ...
        sheets.each do |sheet|
          ...
          reservation = db.xquery('SELECT * FROM reservations WHERE event_id = ? AND sheet_id = ? AND not_canceled=true GROUP BY event_id, sheet_id HAVING reserved_at = MIN(reserved_at)', event['id'], sheet['id']).first
          if reservation
    ...
    get '/api/users/:id', login_required: true do |user_id|
      ...
      rows = db.xquery('SELECT r.*, s.rank AS sheet_rank, s.num AS sheet_num FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ? ORDER BY last_updated_at DESC LIMIT 5', user['id'])
      ...
      user['total_price'] = db.xquery('SELECT IFNULL(SUM(e.price + s.price), 0) AS total_price FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id INNER JOIN events e ON e.id = r.event_id WHERE r.user_id = ? AND r.not_canceled=true', user['id']).first['total_price']
    ...
    post '/api/events/:id/actions/reserve', login_required: true do |event_id|
      ...
      reservation_id = nil
      loop do
        sheet = db.xquery('SELECT * FROM sheets WHERE id NOT IN (SELECT sheet_id FROM reservations WHERE event_id = ? AND not_canceled=true FOR UPDATE) AND `rank` = ? ORDER BY RAND() LIMIT 1', event['id'], rank).first
    ...
    delete '/api/events/:id/sheets/:rank/:num/reservation', login_required: true do |event_id, rank, num|
      ...
      db.query('BEGIN')
      begin
        reservation = db.xquery('SELECT * FROM reservations WHERE event_id = ? AND sheet_id = ? AND not_canceled=true GROUP BY event_id HAVING reserved_at = MIN(reserved_at) FOR UPDATE', event['id'], sheet['id']).first
        unless reservation
          db.query('ROLLBACK')
